...
type
  IllWill* = ref object
    initialized, fullscreen, mouse, fullRedrawNextFrame, doubleBufferingEnabled: bool
    mouseInfo, lastMouseInfo: MouseInfo
    prevTerminalBuffer: TerminalBuffer
    currBg: BackgroundColor
    currFg: ForegroundColor
    currStyle: set[Style]

proc newIllWill*: IllWill =
  var iw = IllWill(
    initialized: false,
    fullscreen: false,
    mouse: false,
    fullRedrawNextFrame: false,
    doubleBufferingEnabled: true
  )
  return iw

proc getMouse*(iw: IllWill): MouseInfo =
...
when defined(windows):
  ...
  proc SIGCONT_handler(iw: IllWill, sig: cint) {.noconv.} =
    signal(SIGCONT, SIGCONT_handler)
    signal(SIGTSTP, SIGTSTP_handler)

    iw.fullRedrawNextFrame = true
    consoleInit()
    hideCursor()
  ...
  proc splitInputs(iw: IllWill, inp: openarray[int], max: Natural): seq[seq[int]] =
    ...
    for ch in inp[CSI.len+1 .. max-1]:
      if ch == ord('M'):
        # Button press
        parts.add(cur)
        iw.mouseInfo.action = mbaPressed
        break
      elif ch == ord('m'):
        # Button release
        parts.add(cur)
        iw.mouseInfo.action = mbaReleased
        break
      elif ch != ord(';'):
        cur.add(ch)
      else:
  ...
  proc fillGlobalMouseInfo(iw: IllWill, keyBuf: array[KeySequenceMaxLen, int]) =
    let parts = splitInputs(keyBuf, keyBuf.len)
    iw.mouseInfo.x = parts[1].getPos() - 1
    iw.mouseInfo.y = parts[2].getPos() - 1
    let bitset = parts[0].getPos()
    iw.mouseInfo.ctrl = bitset.testBit(4)
    iw.mouseInfo.shift = bitset.testBit(2)
    iw.mouseInfo.move = bitset.testBit(5)
    case ((bitset.uint8 shl 6) shr 6).int
    of 0: iw.mouseInfo.button = MouseButton.mbLeft
    of 1: iw.mouseInfo.button = MouseButton.mbMiddle
    of 2: iw.mouseInfo.button = MouseButton.mbRight
    else:
      iw.mouseInfo.action = MouseButtonAction.mbaNone
      iw.mouseInfo.button = MouseButton.mbNone # Move sends 3, but we ignore
    iw.mouseInfo.scroll = bitset.testBit(6)
    if iw.mouseInfo.scroll:
      # on scroll button=3 is reported, but we want no button pressed
      iw.mouseInfo.button = MouseButton.mbNone
      if bitset.testBit(0): iw.mouseInfo.scrollDir = ScrollDirection.sdDown
      else: iw.mouseInfo.scrollDir = ScrollDirection.sdUp
    else:
      iw.mouseInfo.scrollDir = ScrollDirection.sdNone

  proc parseKey(iw: IllWill, charsRead: int): Key =
    # Inspired by
  ...
  proc getKeyAsync(iw: IllWill): Key =
    var i = 0
...
proc illwillInit*(iw: IllWill, fullScreen: bool = true, mouse: bool = false) =
  ...
  iw.fullscreen = fullScreen
  if iw.fullscreen: enterFullScreen()
...
proc checkInit(iw: IllWill) =
  if not iw.initialized:
    raise newException(IllwillError, "Illwill not initialised")

proc illwillDeinit*(iw: IllWill) =
  ...
  checkInit(iw)
  if iw.fullscreen: exitFullScreen()
  if iw.mouse:
    when defined(posix):
      disableMouse()
    else:
...
when defined(windows):
  ...
  proc fillGlobalMouseInfo(iw: IllWill, inputRecord: INPUT_RECORD) =
    iw.mouseInfo.x = inputRecord.Event.MouseEvent.dwMousePosition.X
    iw.mouseInfo.y = inputRecord.Event.MouseEvent.dwMousePosition.Y

    case inputRecord.Event.MouseEvent.dwButtonState
    of FROM_LEFT_1ST_BUTTON_PRESSED:
      iw.mouseInfo.button = mbLeft
    of FROM_LEFT_2ND_BUTTON_PRESSED:
      iw.mouseInfo.button = mbMiddle
    of RIGHTMOST_BUTTON_PRESSED:
      iw.mouseInfo.button = mbRight
    else:
      iw.mouseInfo.button = mbNone

    if iw.mouseInfo.button != mbNone:
      iw.mouseInfo.action = MouseButtonAction.mbaPressed
    elif iw.mouseInfo.button == mbNone and iw.lastMouseInfo.button != mbNone:
      iw.mouseInfo.action = MouseButtonAction.mbaReleased
    else:
      iw.mouseInfo.action = MouseButtonAction.mbaNone

    if iw.lastMouseInfo.x != iw.mouseInfo.x or iw.lastMouseInfo.y != iw.mouseInfo.y:
      iw.mouseInfo.move = true
    else:
      iw.mouseInfo.move = false

    if bitand(inputRecord.Event.MouseEvent.dwEventFlags, MOUSE_WHEELED) == MOUSE_WHEELED:
      iw.mouseInfo.scroll = true
      if inputRecord.Event.MouseEvent.dwButtonState.testBit(31):
        iw.mouseInfo.scrollDir = ScrollDirection.sdDown
      else:
        iw.mouseInfo.scrollDir = ScrollDirection.sdUp
    else:
      iw.mouseInfo.scroll = false
      iw.mouseInfo.scrollDir = ScrollDirection.sdNone

    iw.mouseInfo.ctrl = bitand(inputRecord.Event.MouseEvent.dwControlKeyState, LEFT_CTRL_PRESSED) == LEFT_CTRL_PRESSED or
        bitand(inputRecord.Event.MouseEvent.dwControlKeyState, RIGHT_CTRL_PRESSED) == RIGHT_CTRL_PRESSED

    iw.mouseInfo.shift = bitand(inputRecord.Event.MouseEvent.dwControlKeyState, SHIFT_PRESSED) == SHIFT_PRESSED

    iw.lastMouseInfo = iw.mouseInfo


  proc hasMouseInput(iw: IllWill): bool =
    ...
    if buffer[numberOfEventsRead - 1].EventType == MOUSE_EVENT:
      fillGlobalMouseInfo(iw, buffer[numberOfEventsRead - 1])
      return true
    else:
      return false

proc getKey*(iw: IllWill): Key =
...
proc setAttribs(iw: IllWill, c: TerminalChar) =
  if c.bg == bgNone or c.fg == fgNone or c.style == {}:
    resetAttributes()
    iw.currBg = c.bg
    iw.currFg = c.fg
    iw.currStyle = c.style
    if iw.currBg != bgNone:
      setBackgroundColor(cast[terminal.BackgroundColor](iw.currBg))
    if iw.currFg != fgNone:
      setForegroundColor(cast[terminal.ForegroundColor](iw.currFg))
    if iw.currStyle != {}:
      setStyle(iw.currStyle)
  else:
    if c.bg != iw.currBg:
      iw.currBg = c.bg
      setBackgroundColor(cast[terminal.BackgroundColor](iw.currBg))
...
proc displayFull(iw: IllWill, tb: TerminalBuffer) =
  var buf = ""
  ...
  for y in 0..<tb.height:
    ...
    for x in 0..<tb.width:
      let c = tb[x,y]
      if c.bg != iw.currBg or c.fg != iw.currFg or c.style != iw.currStyle:
        flushBuf()
        setAttribs(iw, c)
      buf &= $c.ch
    flushBuf()
...
proc displayDiff(iw: IllWill, tb: TerminalBuffer) =
  var
    buf = ""
  ...
  for y in 0..<tb.height:
    ...
    for x in 0..<tb.width:
      let c = tb[x,y]
      if c != iw.prevTerminalBuffer[x,y] or c.forceWrite:
        if c.bg != iw.currBg or c.fg != iw.currFg or c.style != iw.currStyle:
          flushBuf()
          bufXPos = x
          setAttribs(iw, c)
...
proc setDoubleBuffering*(iw: IllWill, enabled: bool) =
  ## Enables or disables double buffering (enabled by default).
  iw.doubleBufferingEnabled = enabled
  iw.prevTerminalBuffer = nil

proc hasDoubleBuffering*(iw: IllWill): bool =
  ## Returns `true` if double buffering is enabled.
  ##
  ## If the module is not intialised, `IllwillError` is raised.
  checkInit(iw)
  result = iw.doubleBufferingEnabled

proc display*(iw: IllWill, tb: TerminalBuffer) =
  ## Outputs the contents of the terminal buffer to the actual terminal.
  ##
  ## If the module is not intialised, `IllwillError` is raised.
  checkInit(iw)
  if not iw.fullRedrawNextFrame and iw.doubleBufferingEnabled:
    if iw.prevTerminalBuffer == nil:
      displayFull(iw, tb)
      iw.prevTerminalBuffer = newTerminalBufferFrom(tb)
    else:
      if tb.width == iw.prevTerminalBuffer.width and
         tb.height == iw.prevTerminalBuffer.height:
        displayDiff(iw, tb)
        iw.prevTerminalBuffer.copyFrom(tb)
      else:
        displayFull(iw, tb)
        iw.prevTerminalBuffer = newTerminalBufferFrom(tb)
    flushFile(stdout)
...
when isMainModule:
  ...
  proc test() {.thread.} =
    ...
    illwillInit(iw, fullscreen=true)
    # setControlCHook(exitProc)
    hideCursor()
    var tb = newTerminalBuffer(terminalWidth(), terminalHeight())
